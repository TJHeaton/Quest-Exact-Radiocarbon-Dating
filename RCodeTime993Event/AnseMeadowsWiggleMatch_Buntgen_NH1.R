# Wiggle matching L'Anse aux Meadows Beams vs Buntgen NH1 reference data
...
# Wiggle matching L'Anse aux Meadows over a MiyakeCal curve using Buntgen NH0 data
MiyakeCal <- read.csv("CalibrationCurves/BuntgenNH1_Miyake993ADCalCurve.csv", header = TRUE)
...
# Function which calibrates multiple samples in a wiggle match
# Arguments:
...
# Plot Miyake curve
lines(MiyakeCal$AD, MiyakeCal$c14age, col = NH1_col)
polygon(c(MiyakeCal$AD, rev(MiyakeCal$AD)),
        c(MiyakeCal$C14Upper, rev(MiyakeCal$C14Lower)),
        col = adjust_transparency(NH1_col, alpha = 0.5),
        border=NA)
...
legend("topright", legend = c("BuntgenCal - NH1 Only"), 
       lty = c(-1), pch = c(15), lwd = 2,
       col = c(adjust_transparency(NH1_col, alpha = 0.5)), cex = 0.8, pt.cex = pt_cex)
...
(max_cal_age_prob <- max(probs)) # In this case 97.8%
(max_cal_age_fit <- 1950 - fromto[which.max(probs)]) # In this case 1021 cal AD


text(x = 1021, y = max(pol[,2]), 
     labels = "97.5% 1021 cal AD", 
     cex = 0.8, pos = 2)
